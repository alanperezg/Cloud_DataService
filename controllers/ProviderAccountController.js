const providerAccountActions = require ('../utils/providerAccountActions')
const jwt = require('jsonwebtoken');

const getTokenByAuth = async (req, res) => {
    const body = req.body;
    if(body.user !== undefined && body.password !== undefined){
        const clientData = await providerAccountActions.getAccountByAuth({user: body.user, password: body.password})
